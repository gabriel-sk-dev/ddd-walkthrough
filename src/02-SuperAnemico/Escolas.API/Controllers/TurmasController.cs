    ...
    public class TurmasController : ControllerBase
    {
        ...
        public IActionResult Nova([FromBody] Turma novaTurma)
        {
            try
            {
                ...
                var sql = "INSERT INTO Turmas (Id, Descricao, LimiteAlunos, IdadeMinima, Aberta) VALUES (@Id, @Descricao, @LimiteAlunos, @IdadeMinima, @Aberta)";
                using (var conexao = new SqlConnection(_configuracao.GetConnectionString("Escolas")))
                {
                    novaTurma.Id = Guid.NewGuid().ToString();
                    var resultado = conexao.Execute(sql, new { novaTurma.Id, novaTurma.Descricao, novaTurma.LimiteAlunos, novaTurma.IdadeMinima, novaTurma.Aberta });
                    if (resultado <= 0)
        ...
        public IActionResult Recuperar(string id)
        {
            try
            {
                var sql = "SELECT Id, Descricao, LimiteAlunos, IdadeMinima, Aberta FROM Turmas WHERE Id = @id";
                using (var conexao = new SqlConnection(_configuracao.GetConnectionString("Escolas")))
                {
